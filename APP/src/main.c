...
#define lum_threshold 251

//////////////////////////////////////
/////////////// AX 12 ////////////////
//////////////////////////////////////

// infinite turn mode activation
...
// infinite turn mode desactivation
// parameter: ID of motor
...
// returns the obstacle detection flag (using infrared sensors)
...
// returns the light detection flag (using visual light sensors)
// parameter sensor: Id of AX-S1
// parameter boolLight: pointer to store data read from AX-S1
...
// returns the left infrared reading.
// parameter sensor: Id of AX-S1
...
// returns the center infrared reading. 
// parameter sensor: Id of AX-S1
// parameter sideIR: pointer to store data read from AX-S1
...
// returns the right infrared reading. 
// parameter sensor: Id of AX-S1
// parameter sideIR: pointer to store data read from AX-S1
...
// returns the left leight sensor reading. 
// parameter sensor: Id of AX-S1
...
// returns the central light sensor reading. 
// parameter sensor: Id of AX-S1
...
// returns the right light sensor reading. 
// parameter sensor: Id of AX-S1
...
int main(void)
{
  ...
  int thresholdInfrared = 3 ;

  int state;
	unsigned char left_lum;
  ...
  while(state!=STOP)
  {
     ...
     // play some music
buzzWithDelay(SENSOR,22,200);
buzzWithDelay(SENSOR,22,200);
buzzWithDelay(SENSOR,15,200);
buzzWithDelay(SENSOR,18,200);
buzzWithDelay(SENSOR,18,200);
mDelay(250);
buzzWithDelay(SENSOR,23,200);
buzzWithDelay(SENSOR,23,200);
mDelay(100);
buzzWithDelay(SENSOR,17,200);
buzzWithDelay(SENSOR,20,200);
mDelay(100);
buzzWithDelay(SENSOR,22,200);
buzzWithDelay(SENSOR,22,200);
buzzWithDelay(SENSOR,15,200);
...
mDelay(100);
	...
	rightInfraRed(SENSOR,&left_lum);
	if (left_lum >= lum_threshold)
		state = BORD;
